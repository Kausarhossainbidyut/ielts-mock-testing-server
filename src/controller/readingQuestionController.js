...
// Get all reading questions with filtering
exports.getAllQuestions = async (req, res, next) => {
  try {
    ...
    const questions = await ReadingQuestion.find(filter)
    ...
    const total = await ReadingQuestion.countDocuments(filter);
...
exports.getQuestionById = async (req, res, next) => {
  try {
    ...
    if (!question) {
      return next(createError(404, "Reading question not found"));
    }
...
// Create new reading question (admin only)
exports.createQuestion = [
  ...
  body("type")
    .isIn(["multiple-choice", "true-false-not-given", "yes-no-not-given", "matching-headings", "matching-information", "matching-sentence-endings", "sentence-completion", "summary-completion", "table-completion", "flow-chart-completion", "diagram-label-completion", "short-answer"])
    .withMessage("Invalid reading question type"),
  body("passage")
    .notEmpty()
    .withMessage("Reading passage is required"),
  body("content")
    .notEmpty()
...
// Update reading question (admin only)
exports.updateQuestion = [
  ...
  body("passage")
    .optional()
    .notEmpty()
    .withMessage("Reading passage cannot be empty"),
  ...
  async (req, res, next) => {
    try {
      ...
      const existingQuestion = await ReadingQuestion.findById(id);
      if (!existingQuestion) {
        return next(createError(404, "Reading question not found"));
      }
      ...
      if (req.body.questionNumber && req.body.questionNumber !== existingQuestion.questionNumber) {
        const conflict = await ReadingQuestion.findOne({
...
// Delete reading question (admin only)
...
// Get random reading questions for practice
exports.getRandomQuestions = async (req, res, next) => {
  try {
    ...
    const questions = await ReadingQuestion.aggregate([
      { $match: filter },
